...
export default class App extends Component {
  ...
  handleSongPlaying(audio) {
    this.setState({
      elapsed: this.parseMillisecons(audio.position),
      total: this.parseMillisecons(audio.duration),
  ...
  parseMillisecons(milliseconds) {
    ...
    let secconds = Math.floor(milliseconds / 1000)
    milliseconds = Math.floor(milliseconds % 1000)

    return(
      (minutes < 10 ? '0' : '') + minutes + ':' +
      (secconds < 10 ? '0' : '') + secconds
    )
  }
  ...
  randomTrack() {
    ...
    Axios.get('https://api.soundcloud.com/users/134064959/favorites?client_id='+this.clientId)
      .then((res) => {
        const trackLength = res.data.length
  ...
  togglePlay() {
    if(this.state.playStatus === Sound.status.PLAYING) {
      this.setState({playStatus: Sound.status.PLAUSE})
    } else {
  ...
  render() {
    ...
    return (
      <div style={playStyle} class='ui fluid container'>
        <div class='ui one column centerd grid'>
          <div class="column">
          <Sound 
            ...
            onFinishedPlaying={this.handleSongFinised.bind(this)}/>
          </div>
          <div class="column">
            <Search
